...
public class DropCollection implements MongoCdcOperation {
    ...
    public Either<WriteModel<BsonDocument>, Consumer<MongoClient>> process(final SinkDocument doc) {
        BsonDocument changeStreamDocument = doc.getValueDoc().orElseThrow(
                () -> new DataException("Error: value doc must not be missing for drop collection operation")
        );

        return Either.right(mongoClient -> mongoClient
                .getDatabase(getDatabaseName(changeStreamDocument))
                .getCollection(getCollectionName(changeStreamDocument))
                .drop());
    }
}
