...
import static com.mongodb.kafka.connect.source.MongoSourceConfig.INITIAL_SYNC_MAX_THREADS_CONFIG;
 ...
 *
 * <ol>
 * <li>Gets all namespaces to sync. eg. A single collection, all collections in a database or all collections in all databases.</li>
 * <li>For each namespace, submit to the executors an initial sync task.</li>
 * <li>Each initial sync task, runs an aggregation pipeline on the namespace, to mimic an insert document and adds the results to the queue.</li>
 * <li>The poll method returns documents from the queue.</li>
 * </ol>
 */
class MongoInitialSync implements AutoCloseable {
    private static final Logger LOGGER = LoggerFactory.getLogger(MongoInitialSync.class);
    private final AtomicBoolean closed = new AtomicBoolean();
    private final AtomicInteger namespacesToSync;
    ...
    MongoInitialSync(final MongoSourceConfig sourceConfig, final MongoClient mongoClient) {
        ...
        LOGGER.info("Performing initial sync on the following namespaces: {}", namespaces);
        namespacesToSync = new AtomicInteger(namespaces.size());
        executor = Executors.newFixedThreadPool(
                Math.min(namespacesToSync.get(), sourceConfig.getInt(INITIAL_SYNC_MAX_THREADS_CONFIG)));
        namespaces.forEach(n -> executor.submit(() -> initialSyncCollection(n)));
    }

    Optional<BsonDocument> poll() {
        if (namespacesToSync.get() == 0) {
            close();
        }
    ...
    boolean isSyncing() {
        return namespacesToSync.get() > 0 || !queue.isEmpty();
    }
    ...
    private void initialSyncCollection(final MongoNamespace namespace) {
        LOGGER.debug("Performing initial sync on: {}", namespace.getFullName());
