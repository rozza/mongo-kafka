...
class MongoInitialSyncTest {
    ...
    void testReturnsTheExpectedCollectionResults() {
        BsonDocument result = BsonDocument.parse("{'_id': {'_id': 1, 'initialSync': true}, "
        ...
        try (MongoInitialSync initialSync = new MongoInitialSync(createSourceConfig(), mongoClient)) {
            sleep();
            results = asList(initialSync.poll(), initialSync.poll());
        }
    ...
    void testReturnsTheExpectedDatabaseResults() {
        BsonDocument myDbColl1Result = BsonDocument.parse("{'_id': {'_id': 1, 'initialSync': true}, "
                + "'operationType': 'insert', 'ns': {'db': 'myDB', 'coll': 'coll1'}, "
                + "'documentKey': {'_id': 1}, "
                + "'fullDocument': {'_id': 1, 'a': 'a', 'b': 121}}");

        BsonDocument myDbColl2Result = BsonDocument.parse("{'_id': {'_id': 2, 'initialSync': true}, "
        ...
        List<Optional<BsonDocument>> results;
        try (MongoInitialSync initialSync = new MongoInitialSync(new MongoSourceConfig(dbConfig), mongoClient)) {
            sleep();
            results = asList(initialSync.poll(), initialSync.poll(), initialSync.poll());
        }
    ...
    void testReturnsTheExpectedClientResults() {
        BsonDocument db1Coll1Result1 = BsonDocument.parse("{'_id': {'_id': 1, 'initialSync': true}, "
                + "'operationType': 'insert', 'ns': {'db': 'db1', 'coll': 'coll1'}, "
                + "'documentKey': {'_id': 1}, "
                + "'fullDocument': {'_id': 1, 'a': 'a', 'b': 121}}");
        BsonDocument db1Coll1Result2 = BsonDocument.parse("{'_id': {'_id': 2, 'initialSync': true}, "
                + "'operationType': 'insert', 'ns': {'db': 'db1', 'coll': 'coll1'}, "
                + "'documentKey': {'_id': 2}, "
                + "'fullDocument': {'_id': 2, 'a': 'aa', 'b': 111}}");
        BsonDocument db2Coll2Result1 = BsonDocument.parse("{'_id': {'_id': 1, 'initialSync': true}, "
        ...
        List<Optional<BsonDocument>> results;
        try (MongoInitialSync initialSync = new MongoInitialSync(new MongoSourceConfig(new HashMap<>()), mongoClient)) {
            sleep();
            results = asList(initialSync.poll(), initialSync.poll(), initialSync.poll(), initialSync.poll());
        }
